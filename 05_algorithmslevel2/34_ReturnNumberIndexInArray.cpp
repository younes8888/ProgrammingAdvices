...
short SearchNumPositionAndOrderInArray(int Arr1[100], int ArrLength, int NumToSearch)
{
    for (int i = 0; i < ArrLength; i++)
    {
       if (Arr1[i] == NumToSearch)
           return i;
    }
    return -1;
}
...
int main()
{
    ...
    short NumberPosition = SearchNumPositionAndOrderInArray(Arr1, ArrLength, NumToSearch);
    if (NumberPosition == -1)
    std::cout << "\nThe number is not found: " << std::endl;
    else
    {
        std::cout << "The number four at position: " << NumberPosition << std::endl;
        std::cout << "Then number found its order is: " << NumberPosition + 1 << std::endl;
    }
